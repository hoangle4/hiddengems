...
const MyMapComponent = compose(
	...
	lifecycle({
		componentWillMount() {
			...
			this.setState({
				...
				onPositionChanged: () => {
					const position =
						refs.streetMap.context.__SECRET_MAP_DO_NOT_USE_OR_YOU_WILL_BE_FIRED.streetView.position;
					return {
						lat: position.lat(),
						lng: position.lng()
					};
	...
	<Fragment>
		{props.comReady ? (
			...
			>
				<button onClick={props.handleAddStory} title="Add Story Here" id="MyMapComponent_add_btn">
					<FaPlus />
				</button>
				<button onClick={props.handleStreetView} title="Street View" className="MyMapComponent_switch_btn">
					<FaStreetView />
				</button>
				{props.isStreetView ? (
					<StreetViewPanorama
